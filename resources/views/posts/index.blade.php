...
@section('content')
        <h1>Blog Akbar
            <a href="{{ url('posts/create') }}" class="btn btn-success">(+) Post</a>
        </h1>
        @foreach ($posts as $post)
            {{-- CARA MENAMPILKAN DATA DENGAN TXT --}}
            {{-- <!-- memisahkan string menjadi array dengan "," ==> koma sebagai pemisah -->
            @php($post = explode(',', $post)) --}}
            {{-- ISI VALUE NYA $post[2] --}}
            <div class="card mb-3">
                <div class="card-body">
